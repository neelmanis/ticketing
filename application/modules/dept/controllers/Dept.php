...
class Dept extends Generic
{
	...
	$list = $this->Mdl_dept->get_datatabless("ticket_departments");
    $data = array();
    $no = $_POST['start'];
    ...
    $output = array(
      "draw" => $_POST['draw'],
      "recordsTotal" => $this->Mdl_dept->count_alls(),
      "recordsFiltered" => $this->Mdl_dept->count_filtereds(),
      "data" => $data,
    );
  ...
  function addAction(){
    ...
    if($this->form_validation->run($this) == FALSE){
      ...
      $insert = $this->Mdl_dept->insert("category_master", $data);
  ...
  function update($id){
    ...
    $result = $this->Mdl_dept->retrieve("category_master",array("id"=>$id));
    if($result == "NA"){
  ...
  function updateAction(){
    ...
    if($this->form_validation->run($this) == FALSE){
      ...
      $update = $this->Mdl_dept->update2("category_master","id",$content['id'], $data);
