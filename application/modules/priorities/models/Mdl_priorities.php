...
class Mdl_priorities extends Mdl_generic {

  var $table = 'priorities';
  ...
  private function _get_datatables_query(){
	...
	if($this->input->post('categoryStatus')== "1"){
        $this->db->where('status', $this->input->post('categoryStatus'));
    } else if($this->input->post('categoryStatus')== "0"){
		 $this->db->where('status', $this->input->post('categoryStatus'));
	} 
